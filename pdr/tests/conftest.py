...
def make_product(dir: Path, name: str, content: Union[np.ndarray, bytes, str],
                 label: str, **extra_label_params: Union[str, int]):
...
def uniband_image_product(products_dir):
    ...
    return make_product(products_dir, "UB-IMG-PROD",
                        zeros, STUB_IMAGE_LABEL,
                        bands=1)
...
def multiband_image_product(products_dir):
    ...
    return make_product(products_dir, "MB-IMG-PROD",
                        zeros, STUB_IMAGE_LABEL,
                        bands=3)
...
def binary_table_product(products_dir):
    ...
    return make_product(products_dir, "BIN-TBL-PROD",
                        table, STUB_BINARY_TABLE_LABEL)


@pytest.fixture(scope="session")
def dsv_table_product(products_dir):
    table = "5.5| cat| -12\r\n" * 10
    return make_product(products_dir, "DSV-TBL-PROD",
                        table, STUB_DSV_TABLE_LABEL)
