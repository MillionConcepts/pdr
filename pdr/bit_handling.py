...
def set_bit_string_data_type(obj):
    try:
...
def get_bit_column_objects(definition):
    list_of_pvl_objects_for_bit_columns = definition.getall("BIT_COLUMN")
    return list_of_pvl_objects_for_bit_columns


def get_bit_start_and_size(list_of_pvl_objects_for_bit_columns, obj):
    start_bit_list = []
    bit_size_list = []
    ...
    return start_bit_list


def set_start_bit_list(start_bit_list, obj):
...
def add_bit_column_info(obj, definition):
    if "BIT_COLUMN" in obj.keys():
        if "BIT_STRING" not in obj["DATA_TYPE"]:
            obj = specialize(set_bit_string_data_type, check_special_bit_start_case)
        obj = get_bit_start_and_size(obj, definition)
    return obj
